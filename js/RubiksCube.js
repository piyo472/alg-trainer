...
var recapMode = 0; // 0: Off, 1: On (Once), 2: Repeat
var recapQueue = [];
var recapInitialSize = 0;

var skipZBLSMode = false;
var savedSkipZBLS = localStorage.getItem('skipZBLSMode');
if (savedSkipZBLS !== null) {
    skipZBLSMode = (savedSkipZBLS === 'true');
}
...
var connectBtn = document.getElementById("connectSmartCube");
var disconnectBtn = document.getElementById("disconnectSmartCube");
var statusSpan = document.getElementById("smartCubeStatus");
var batteryDiv = document.getElementById("smartCubeBattery");
...
connectBtn.addEventListener('click', function () {
    ...
    SmartCubeBridge.connect().catch(function (e) {
        statusSpan.textContent = 'Connection Failed: ' + e;
...
disconnectBtn.addEventListener('click', function () {
    SmartCubeBridge.disconnect();
});

SmartCubeBridge.onConnect(function (deviceName) {
    statusSpan.textContent = 'Connected: ' + (deviceName || 'Smart Cube');
    statusSpan.style.color = 'green';
    connectBtn.style.display = 'none';
    disconnectBtn.style.display = 'inline-block';

    // Automatically turn on virtual cube when connecting a smart cube
    setVirtualCube(true);
    var useVirtualCheckbox = document.getElementById("useVirtual");
    if (useVirtualCheckbox) {
        useVirtualCheckbox.checked = true;
        localStorage.setItem("useVirtual", true);
    }

    batteryDiv.style.display = 'block';
    SmartCubeBridge.getBatteryLevel().then(function (level) {
        batteryDiv.textContent = 'Battery: ' + level + '%';
    }).catch(function () {
        batteryDiv.textContent = 'Battery: N/A';
    });
});

SmartCubeBridge.onDisconnect(function () {
...
SmartCubeBridge.onMove(function (moveStr) {
    doAlg(moveStr);
});
...
var continuousTraining = document.getElementById("continuousTraining");
if (continuousTraining) {
    continuousTraining.addEventListener("click", function () {
        localStorage.setItem("continuousTraining", this.checked);
    });
}
...
var postScrambleRotation = document.getElementById("postScrambleRotation");
if (postScrambleRotation) {
    postScrambleRotation.addEventListener("input", function () {
        localStorage.setItem("postScrambleRotation", this.value);
    });
}
...
function toggleRecap(forceOff) {
    if (forceOff === true) {
        recapMode = 0;
    } else {
        recapMode = (recapMode + 1) % 3;
    }

    var btn = document.getElementById("recapButton");
    var status = document.getElementById("recapStatus");

    if (recapMode === 0) {
        btn.innerText = "Recap: Off";
        ...
        if (recapMode === 1) {
            btn.innerText = "Recap: On";
        } else if (recapMode === 2) {
            btn.innerText = "Recap: Repeat";
        }
        btn.style.color = "blue";

        if (recapQueue.length === 0 || recapMode === 2) {
            var currentSet = createAlgList(true); // pass true to get list properly
            for (let i = currentSet.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [currentSet[i], currentSet[j]] = [currentSet[j], currentSet[i]];
            }
            recapQueue = currentSet;
            recapInitialSize = recapQueue.length;
            status.innerText = recapInitialSize + "/" + recapInitialSize;
            nextScramble();
        }
    }
}

function updateRecapQueue() {
    var cb = document.getElementById("recapButton");
    if (!cb || cb.innerText === "Recap: Off") return;

    var currentSet = createAlgList(true); // Make sure it gets algs right
    ...
    var recapStatusEl = document.getElementById("recapStatus");
    if (recapStatusEl) {
        recapStatusEl.innerText = recapInitialSize + "/" + recapInitialSize;
    }
    nextScramble();
}

function toggleSkipZBLS() {
    skipZBLSMode = !skipZBLSMode;
    localStorage.setItem('skipZBLSMode', skipZBLSMode);
    updateSkipZBLSButton();
}

function updateSkipZBLSButton() {
    var btn = document.getElementById("skipZBLLButton");
    if (!btn) return;
    var algsetPicker = document.getElementById("algsetpicker");
    var currentSet = algsetPicker ? algsetPicker.value : "";
    var isZBLSSet = currentSet.toLowerCase().includes("zbls") || currentSet.toLowerCase().includes("vhls");

    btn.innerText = "Skip ZBLS: " + (skipZBLSMode ? "On" : "Off");

    if (!isZBLSSet) {
        btn.style.color = "gray";
    } else {
        btn.style.color = skipZBLSMode ? "blue" : "black";
    }
}

var recapButton = document.getElementById("recapButton");
if (recapButton) recapButton.addEventListener('click', toggleRecap);

var skipZBLLButton = document.getElementById("skipZBLLButton");
if (skipZBLLButton) {
    skipZBLLButton.addEventListener('click', toggleSkipZBLS);
    updateSkipZBLSButton();
}
...
algsetpicker.addEventListener("change", function () {
    ...
    if (recapMode !== 0) updateRecapQueue();
    updateSkipZBLSButton();
});
...
function doAlg(algorithm) {
    ...
    if (timerIsRunning && cube.isSolved() && isUsingVirtualCube()) {
        stopTimer();
        if (document.getElementById("continuousTraining") && document.getElementById("continuousTraining").checked) {
            setTimeout(function () {
                nextScramble();
            }, 50);
        }
...
function generateAlgTest() {
    ...
    var postScrambleRotationObj = document.getElementById("postScrambleRotation");
    if (postScrambleRotationObj && postScrambleRotationObj.value) {
        scramble += " " + postScrambleRotationObj.value;
...
function randomFromList(set) {
    if (recapMode !== 0 && recapQueue.length > 0) {
        var alg = recapQueue.pop();
        var remaining = recapQueue.length;
        document.getElementById("recapStatus").innerText = (remaining + 1) + "/" + recapInitialSize;
        return alg;
    } else if (recapMode !== 0 && recapQueue.length === 0) {
        if (recapMode === 1) {
            toggleRecap(true);
        } else if (recapMode === 2) {
            var currentSet = createAlgList(true);
            for (let i = currentSet.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [currentSet[i], currentSet[j]] = [currentSet[j], currentSet[i]];
            }
...
function RubiksCube() {
    ...
    this.isSolved = function () {
        var algsetPicker = document.getElementById("algsetpicker");
        var currentSet = algsetPicker ? algsetPicker.value : "";
        var isZBLSList = skipZBLSMode && (currentSet.toLowerCase().includes("zbls") || currentSet.toLowerCase().includes("vhls"));

        if (isZBLSList) {
            var dColor = this.cubestate[31];
            for (var i = 27; i <= 35; i++) {
                if (this.cubestate[i] !== dColor) return false;
            }

            var rColor = this.cubestate[13];
            for (var i = 12; i <= 17; i++) {
                if (this.cubestate[i] !== rColor) return false;
            }

            var fColor = this.cubestate[22];
            for (var i = 21; i <= 26; i++) {
                if (this.cubestate[i] !== fColor) return false;
            }

            var lColor = this.cubestate[40];
            for (var i = 39; i <= 44; i++) {
                if (this.cubestate[i] !== lColor) return false;
            }

            var bColor = this.cubestate[49];
            for (var i = 48; i <= 53; i++) {
                if (this.cubestate[i] !== bColor) return false;
            }

            var uColor = this.cubestate[4];
            if (this.cubestate[1] !== uColor ||
                this.cubestate[3] !== uColor ||
                this.cubestate[5] !== uColor ||
                this.cubestate[7] !== uColor) {
                return false;
            }
